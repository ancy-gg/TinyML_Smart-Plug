...
void VoltageSensor::setWindowMs(uint16_t ms) { if (ms < 60) ms = 60; if (ms > 600) ms = 600; _windowUs = (uint32_t)ms * 1000UL; }
...
void VoltageSensor::setLongAverage(float tauS, float jumpV) { if (tauS < 0.2f) tauS = 0.2f; if (tauS > 8.0f) tauS = 8.0f; if (jumpV < 3.0f) jumpV = 3.0f; _avgTauS = tauS; _avgJumpV = jumpV; }

float VoltageSensor::update() {
  const uint32_t now = micros();
  if (!_sampling) {
    _sampling = true; _startTime = now; _count = 0; _sum = 0.0; _sumSq = 0.0; _sampleMin = 4095; _sampleMax = 0; (void)analogRead(_pin); return -1.0f;
  }

  constexpr int FILTERED_SAMPLES_PER_CALL = 6;
  ...
  const bool signalTooSmall = (vrmsCounts < 5.0f) || (p2pCounts < 65);
  const bool signalShapeInvalid = (!signalTooSmall) && (activityRatio < 1.55f);
  ...
  const bool hardZero = (_rawVrms <= _vOff && _lowWindows >= 1) || (_noSignalWindows >= 1);

  if (_rawVrms <= _vOff) { if (_lowWindows < 255) _lowWindows++; } else _lowWindows = 0;

  if (!_protInit) { _protInit = true; _protVrms = _rawVrms; }
  else {
    const float dProt = fabsf(_rawVrms - _protVrms);
    const bool snapReconnect = (_protVrms <= _vOff) && (_rawVrms >= VOLT_NORMAL_MIN_V);
    const bool fastFaultEvent = (_rawVrms >= VOLT_OV_DELAY_V) || ((_rawVrms >= VOLT_UV_CANDIDATE_RAW_MIN_V) && (_rawVrms < VOLT_NORMAL_MIN_V));
    const bool healthyRecovery = (_protVrms < VOLT_NORMAL_MIN_V && _rawVrms >= VOLT_RECOVER_MIN_V) || (_protVrms > VOLT_NORMAL_MAX_V && _rawVrms <= VOLT_RECOVER_MAX_V);

    if (hardZero) {
      _protVrms = 0.0f;
    } else if (snapReconnect || fastFaultEvent || healthyRecovery || dProt >= _avgJumpV) {
      _protVrms = _rawVrms;
    } else {
      const float alphaProt = fminf(1.0f, dtS / 0.45f);
      _protVrms += alphaProt * (_rawVrms - _protVrms);
      if (_protVrms < 0.25f) _protVrms = 0.0f;
    }
  }

  if (!_dispInit) { _dispInit = true; _dispVrms = _protVrms; return _dispVrms; }

  const float delta = fabsf(_protVrms - _dispVrms);
  if (_protVrms <= 0.0f || _noSignalWindows >= 1) {
    _dispVrms = 0.0f;
  } else if ((_dispVrms <= _vOff && _protVrms >= VOLT_NORMAL_MIN_V) || delta >= 18.0f) {
    _dispVrms = _protVrms;
  } else {
    const float tau = (_protVrms >= _dispVrms) ? 0.18f : 0.30f;
    const float alphaDisp = fminf(1.0f, dtS / tau);
