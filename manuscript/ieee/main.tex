...
\begin{abstract}
Residential electrical-fire prevention extends beyond clearing hard short
circuits. Progressive hazards such as sustained overload, degraded
plug-socket contact heating, and load-limited series arcing may develop
locally before conventional upstream protection directly observes the
fault mechanism. This paper presents a microcontroller-based complementary
socket-level protection architecture that separates the evidence required
for these hazards. The implementation integrates calibrated voltage,
current, and temperature sensing on an ESP32-S3; deterministic persistence logic
for overload; compensated socket-temperature estimation for degraded
contact heating; and load-aware Random Forest TinyML inference for
series arc-fault detection. Relay isolation, fault latching, local
audio-visual alerts, and Firebase/PWA monitoring and labeling support were
implemented, while relay protection remained local to the firmware.
Validation produced mean absolute errors of 3.910V, 0.178A, and
1.02$^{\circ}$C for voltage, current, and thermistor channels,
respectively. The highest-stress overload case averaged 17.30A and
opened the relay after 61.3s. Degraded-socket trials heated faster than
normal-socket trials and reached reference terminal temperatures of
55--56$^{\circ}$C in severe cases before or during relay action. Across
200 balanced normal and arc trials, the embedded arc model with firmware
persistence achieved 97.50\% accuracy, 98.97\% precision, 96.00\% recall,
and 97.46\% F1-score; held-out event-level arc reporting showed 84.0\%
event recall with one false-alarm event. These results support controlled
laboratory feasibility for a complementary embedded protection pipeline,
with certification-oriented testing and listed-device comparison left for
future work.
\end{abstract}

\begin{IEEEkeywords}
TinyML, smart plug, series arc fault detection, socket heating, overload
protection, residential electrical safety.
\end{IEEEkeywords}
...
\cite{gma-bfp,nfpa-electrical}. The engineering problem includes the
progressive region between apparently normal operation and a developed
fire-inducing fault, in addition to catastrophic short circuits.
...
practice address fault classes for which they are designed. However,
ordinary overcurrent protection is coordinated mainly around conductor
thermal protection and high-current events; it lacks direct visibility of
outlet contact temperature and low-current series-arcing evidence
\cite{schneider-inverse,bal-pvc,lee-afci}. At a socket, a poor
plug-contact interface can develop localized heating while the total load
current remains within the ordinary operating range. A load-limited
series arc can also disturb the waveform without producing a large
RMS-current increase.

This study distinguishes three fire-inducing mechanisms. Sustained
overload is a current-persistence and time-stress problem. Degraded
plug-socket contact heating is a local thermal problem, where the hottest
contact region may be hidden from an external thermistor. Series arcing
is a stochastic, load-dependent waveform problem, because normal nonlinear
loads can share some arc-like characteristics. Combining these mechanisms
under one generic abnormal state obscures the relevant evidence: an RMS
current threshold alone cannot represent localized heating or load-limited
arcing.

The objective of this work was to design and evaluate an ESP32-S3-based
socket-level embedded protection prototype for selected progressive
residential fault conditions. Specifically, the work aimed to integrate
calibrated voltage, current, and temperature sensing; implement
deterministic overload and socket-heating protection; deploy load-aware
TinyML series arc inference; provide local relay isolation, fault
latching, and alerts; and support monitoring, event logging, and data
labeling through Firebase and a Progressive Web App (PWA).

This paper contributes a hybrid embedded protection architecture that
separates evidence by fault mechanism: current persistence for overload,
compensated socket-temperature estimation for degraded-contact heating,
and load-context-conditioned waveform-feature inference for series arcs.
The evaluation is framed as a controlled laboratory feasibility study in
electronics and computer engineering (ECE), with socket-level protection
treated as complementary to breakers, arc-fault circuit interrupters
(AFCIs), ground-fault circuit interrupters (GFCIs), wiring inspection,
and code-compliant residential protection.
...
short transient or inrush event is inconsistent with time-current
...
\begin{table}[t]
\caption{Fault Mechanisms and Evidence Used by the Implementation}
\label{tab:evidence}
\centering
\footnotesize
...
\toprule
\textbf{Fault class} & \textbf{Fire-relevant mechanism} & \textbf{Evidence used for protection}\\
\midrule
...
series arc-fault detection. The resulting protection pipeline is local:
if Wi-Fi or Firebase is unavailable, the ESP32-S3 can still evaluate
faults and command relay isolation.

\section{METHODOLOGY}

\subsection{Hardware and Sensing Architecture}

Fig.~\ref{fig:hardware} shows the block-level hardware implementation.
...
influence. Because this point is offset from the hidden contact hotspot, the
...
The relay and latch formed the final local protective actuator. After a
...
history, CSV review, and operator fields, but the relay actuation path was
...
\begin{figure}[t]
  ...
  \caption{Firmware protection flow from sensing and feature extraction to relay actuation and fault clearing.}
...
The principal protection constants are listed in Table~\ref{tab:thresholds}.

\begin{table}[t]
\caption{Principal Firmware Protection Constants}
...
retaining relay closure during warning-level excursions. A sustained trip required the 1s current
average to remain at or above 14.5A long enough for the overload score
to reach six. Since the score increments every 10s, this corresponds to
approximately 60s of persistent overload. This design choice prevents
...
\begin{figure}[H]
  ...
  \caption{TinyML processing sequence from waveform features to local relay action.}
  \label{fig:tinyml}
\end{figure}
...
However, the collected and tested signatures in this study were limited
to resistive loads, inductive loads, and brushed or universal-motor
loads. The context was accepted only when confidence was at least 0.45.
...
loads. The deployed feature set can be summarized in four groups. First,
the conduction-gap inputs--maximum low-current run, zero-dwell ratio,
...
split into training, validation, and held-out test views, and exported as
firmware headers. The final arc report used 35,512 training rows, 9,434
validation rows, and 10,268 held-out test rows. Embedded testing then
evaluated the exported model inside the firmware protection path, where
the model output was filtered by the arc persistence counter before relay
action. Thus, the reported trial result represents the deployed protection
pipeline, including embedded inference and counter-based relay logic.
...
during laboratory trials, but they were supporting infrastructure. The
local ESP32-S3 firmware remained responsible for protection-state
...
\toprule
\textbf{Trial} & \textbf{Load setup} & \textbf{\shortstack{Mean\\A}} & \textbf{\shortstack{Peak\\A}} & \textbf{\shortstack{Relay\\trip}}\\
\midrule
1 & 1.3kW kettle + water heater & 12.87 & 14.85 & No\\
2 & 1.3kW kettle + 1.5kW kettle & 12.33 & 13.63 & No\\
3 & 1.5kW kettle + water heater & 13.36 & 14.86 & No\\
4 & 1.3kW kettle + water heater + 2 lamps & 17.30 & 19.73 & 61.3s\\
5 & 1.3kW kettle + water heater + 1 lamp & 14.42 & 16.23 & No\\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[H]
\caption{Socket-Heating Protection Summary (WH: Water Heater)}
\label{tab:heating}
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
...
\toprule
\textbf{Load case} & \textbf{Socket} & \textbf{\shortstack{Mean\\A}} & \textbf{\shortstack{IR terminal\\$^{\circ}$C}} & \textbf{Relay}\\
\midrule
...
markers, confirming that frame-level arc evidence was sparse and
imbalanced. The final held-out Random Forest test split contained 10,268
feature rows, including 90 positive rows. Frame-level results were
10,173 true negatives, 5 false positives, 25 false negatives, and 65
true positives, corresponding to 99.7078\% accuracy, 92.8571\%
precision, 72.2222\% recall, and 81.25\% F1-score. These row-level
metrics show raw classifier behavior before a complete relay trial is
interpreted.

Event-level metrics provide a stricter view of arc sensitivity over
grouped episodes. The held-out event report contained 25 true arc
events, 21 detected events, 4 missed events, 31 predicted events, 1
false-alarm event, 84.0\% event recall, 96.7742\% event precision, and
89.9358\% event F1-score. These metrics show that some induced arc
episodes were missed even though false event alarms remained low. They
provide the raw event-sensitivity view that complements the relay-facing
trial metrics.
...
was 96.00\%, and F1-score was 97.46\%. Trial-level metrics are higher
than frame-level and event-level metrics because they include the complete
firmware behavior: repeated arc-like frames accumulate toward a relay
decision, while isolated normal outputs decay.
...
Held-out frame & 99.71 & 92.86 & 72.22 & 81.25\\
Held-out event & -- & 96.77 & 84.00 & 89.94\\
\bottomrule
\end{tabular*}
\end{table}

The residual trial-level errors were concentrated in brushed or
...
trials. This behavior is consistent with load-dependent arc-fault
literature: healthy motor commutation, startup, and nonlinear current
features can overlap with arc indicators. The result supports load-aware
inference within the tested socket-level classifier and its evaluated
load-family boundaries.

The trial groups represent the collected load families. Water heater,
halogen lamp, electric kettle, and heat gun cases were treated as
resistive; the 40W stand fan represented the inductive family; and the
handheld vacuum, Electrolux vacuum, angle grinder, bench cutter, and
impact drill represented brushed or universal-motor loads. The dataset
included difficult motor behavior while leaving broader household
electronic-load coverage for future work.

\subsection{Integrated Protection Assessment}

The integrated assessment is that the three protection paths complement
one another. Overload protection addressed persistent current stress
...
This assessment is bounded by the laboratory scope. The prototype
demonstrated controlled feasibility under selected loads, induced faults,
and bench conditions. Certification, field readiness, and full
residential nuisance-immunity evaluation require separate future work.
...
outside comprehensive induced-fault coverage. The reported false positive
and false negative cases show that the dataset remains incomplete,
especially for brushed and nonlinear loads. Reporting both event-level
and trial-level metrics gives a more balanced view of raw sensitivity and
relay-facing behavior.
...
threshold for conservative relay action, not a universal socket safety
limit or direct hotspot measurement.
...
reduced when the network was unavailable. The device provides socket-level
supervision only, without whole-house fault localization.
...
fire-inducing electrical faults. Its ECE contribution is the integration
of calibrated sensing, deterministic protection logic, load-aware
embedded TinyML, relay latching, and monitoring support in one local
protection pipeline. The work also shows why the evidence must be
...
trial-level accuracy with missed-event and false-event cases retained in
the analysis. These findings support feasibility for the proposed
...
certification-oriented testing. Arc-event labeling should also be
improved so future TinyML models can be trained and evaluated with
stronger event-level evidence.

\section*{Acknowledgment}

The authors thank Dr. Referendo D. Soriano for research guidance; Engr.
Anacleto S. Casimiro, Dr. Joel R. Noche, and Engr. Jean Paul I. Paje for
panel review and recommendations; and the laboratory personnel,
electricians, families, and friends who supported safe prototype
