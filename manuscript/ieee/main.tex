...
fire-inducing conditions less directly observed. This study presents a
...
in 230 V residential circuits. The prototype integrated calibrated sensing,
...
Random Forest-based series arc fault inference, fault latching, and relay actuation.
Evaluation included sensor validation, controlled sustained-overload tests,
degraded socket-contact heating tests, and induced series arc fault tests.
Revalidation produced mean absolute errors of
3.910 V, 0.178 A, and 1.02$^{\circ}$C. The highest-stress overload case
reached 17.30 A mean current and disconnected after 61.3 s. Degraded sockets
heated faster than normal sockets: at about 9 A, relay trip occurred at 52 min
for the corroded socket and 120 min for the normal socket; at about 13 A, the
corroded and normal sockets tripped at 5 min and 46 min, respectively. Across
...
feasibility for the TinyML Smart Plug as a complement to mandatory residential
electrical protection.
\end{abstract}

\begin{IEEEkeywords}
ESP32-S3, series arc fault detection, socket-contact heating, sustained overload,
Random Forest, load-context classifier, multi-domain sensing.
\end{IEEEkeywords}

\section{INTRODUCTION}

Electrical faults remain a persistent residential fire concern. Philippine
fire incident reports identify electrical issues as a leading fire cause,
and broader home-fire statistics associate electrical failure or malfunction
with structure fires, injuries, and property losses
\cite{gma-bfp,nfpa-electrical}. Beyond catastrophic short circuits,
progressive electrical hazards may remain energized while producing thermal
or waveform abnormalities before conventional circuit protection operates.
...
practice address their intended fault classes. However, ordinary
overcurrent protection is coordinated mainly around conductor heating and
high-current events; it does not directly detect progressive contact heating
at a socket, persistent sub-trip overload behavior, or low-current series
arc faults below the breaker immediate trip region
\cite{schneider-inverse,bal-pvc,lee-afci}. At the
...
can resemble arc-like waveform disturbances. For this reason, the prototype
does not treat all abnormal conditions as one generic fault state. Each
condition requires protection evidence matched to its physical behavior.

This study designed, developed, and evaluated an
...
230 V residential circuits. It integrated calibrated voltage, current,
and temperature sensing; threshold-based sustained-overload and degraded
socket-contact heating protection; load-context classification;
Random Forest-based series arc fault inference; relay-based load isolation;
fault latching; local audiovisual alerts; Firebase logging; and
Progressive Web App (PWA) monitoring, event
review, and data labeling.

The prototype uses hybrid protection because the three target faults require
different evidence. Sustained overload uses RMS-current level and persistence;
degraded socket-contact heating uses device temperature and compensated
estimated socket temperature; and series arc fault detection uses load
context, waveform features, Random Forest inference, and firmware persistence
logic. Evaluation was conducted under controlled laboratory conditions
involving sustained overload, induced series arc fault, and degraded
socket-contact heating scenarios. The prototype is treated as a
complementary protective layer that operates alongside, not in place of,
circuit breakers, AFCIs, GFCIs, wiring inspection, and code-compliant
...
\midrule
Sustained overload & Thermal stress from persistent load current & RMS-current threshold plus approximately 60 s persistence score\\
Degraded socket-contact heating & Localized poor-contact heating at plug-socket interface & Thermistor input plus compensated estimated socket temperature\\
Series arc fault & Load-limited intermittent conduction and waveform disturbance & Load context, waveform features, Random Forest arc inference, and persistence counter\\
\bottomrule
\end{tabularx}
\end{table}
...
The Firebase-backed PWA layer provides visualization, history review,
labeling, and
...
230 V residential circuit. Its major elements were an ESP32-S3
...
10 k$\Omega$ NTC thermistor, solid-state relay, relay latch, OLED display,
...
for sustained-overload protection, a current-dependent term for socket-temperature
...
history, CSV review, and operator fields, while relay actuation was
implemented locally in firmware and hardware.
...
voltage and current values feed deterministic voltage and sustained-overload
...
If sustained overload, degraded socket-contact heating, or series arc fault
evidence reaches its trip
...
\midrule
Voltage supervision & Outside 200-250 V band & Instant 170/260 V, or 7 s delayed low/high voltage\\
Sustained overload & $I_{\mathrm{rms}}>10.0$ A warning & 1 s average current $\geq$14.5 A for about 60 s\\
Degraded socket-contact heating & 39.0$^{\circ}$C estimated temperature & Estimated socket temperature $\geq$40.0$^{\circ}$C\\
Series arc fault & Context-aware Random Forest output & Arc counter reaches 8; +2 for arc frame, -6 for normal frame\\
\bottomrule
\end{tabularx}
\end{table}

Degraded socket-contact heating had highest reported-fault priority because temperature
is the most direct local fire-risk indicator. Series arc fault was evaluated
next because a series arc may be hazardous without sustained overcurrent.
Sustained overload remained a relay-trip condition after the persistence criterion
...
\subsection{Sustained Overload Protection Method}

Sustained-overload testing used high-power load combinations connected through the
prototype. The firmware issued warning-level alerts above 10.0 A while
retaining relay closure during warning-level excursions. A sustained trip
required the 1 s current average to remain at or above 14.5 A long
enough for the overload score to reach six. Since the score increments
every 10 s, this corresponds to approximately 60 s of persistent
...
\subsection{Socket-Contact Heating Method}

Socket-contact heating evaluation used normal and intentionally degraded socket
...
falling below the expected normal socket-temperature rise for the same load while
...
\subsection{TinyML and Series Arc Fault Method}

Series arc fault detection used a two-stage load-aware TinyML pipeline. The
...
current frames at a 27 kHz target sampling rate with a 256-sample hop. A
...
firmware headers. Fig.~\ref{fig:data-deploy} summarizes this process from
...
relay logic. Unlike an offline classifier report, the relay result includes
sensing, feature extraction, context gating,
model output, persistence scoring, and latch actuation.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{fig_ch3_data_labeling_deploy.pdf}
  \caption{Data labeling, model export, and embedded deployment process.}
...
\begin{table}[H]
\caption{Series Arc Fault Dataset Preparation Summary}
...
The PWA and Firebase backend supported laboratory review and labeling by showing
...
Voltage MAE was 3.910 V, current MAE was 0.178 A, and temperature MAE was
...
separation between the 10.0 A warning threshold and 14.5 A sustained-trip
threshold. The temperature result validates the external thermistor
measurement point only; the socket-contact heating protection result is
...
\midrule
Voltage & 3.910 V & $15.539\mathrm{V}^2$ & 3.94 V\\
Current & 0.178 A & $0.04076\mathrm{A}^2$ & 0.202 A\\
Temperature & 1.02$^{\circ}$C & $1.142^{\circ}\mathrm{C}^2$ & 1.07$^{\circ}$C\\
\bottomrule
\end{tabular*}
\end{table}

\subsection{Sustained Overload Protection}

Table~\ref{tab:overload} gives the sustained-overload evaluation. All heavy-load
trials exceeded the warning threshold, while relay operation occurred in
the highest-stress case. Trial 4 averaged 17.30 A, reached a 19.73 A peak,
and opened the relay after 61.3 s. That trip time agrees with the intended
approximately 60 s persistence behavior. Trials 1, 3, and 5 reached peaks
near or above the sustained threshold, then fell below the criterion
before accumulating the required persistence score. Thus, the sustained-overload
...
\begin{table}[!htbp]
\caption{Sustained Overload Evaluation Results}
\label{tab:overload}
\centering
\small
\setlength{\tabcolsep}{3pt}
...
\midrule
1 & 1.3 kW kettle + WH & 12.87 & 14.85 & No\\
2 & 1.3 kW kettle + 1.5 kW kettle & 12.33 & 13.63 & No\\
3 & 1.5 kW kettle + WH & 13.36 & 14.86 & No\\
4 & 1.3 kW kettle + WH + 2 lamps & 17.30 & 19.73 & 61.3 s\\
5 & 1.3 kW kettle + WH + 1 lamp & 14.42 & 16.23 & No\\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Socket-Contact Heating Protection}

Socket-contact heating characterization showed that contact condition changed the
...
\begin{figure}[!htbp]
  ...
  \caption{Water-heater-only low-current socket-contact heating protection validation.}
...
\begin{figure}[!htbp]
  ...
  \caption{Water heater plus four halogen lamps during high-current socket-contact heating protection validation.}
  \label{fig:heating}
\end{figure}

Table~\ref{tab:heating} summarizes all socket-contact heating protection trials.
At about 5 A in the water-heater-only case, neither socket tripped:
...
9 A, the corroded socket reached the 38.2$^{\circ}$C comparison point in
48 min, compared with 105 min for the normal socket. At about 11 A, the
corroded socket reached the same point in 7 min, compared with 52 min for
the normal socket. In the severe 13 A corroded case, the relay tripped
after 5 min and the reference terminal reached 56.0$^{\circ}$C. These
results support the premise that socket-contact heating cannot be inferred from
...
\begin{table}[!htbp]
\caption{Socket-Contact Heating Protection Summary}
\label{tab:heating}
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
...
logged feature records, as shown earlier in Table~\ref{tab:arc-data-prep}.
This imbalance supports the design choice of
...
\shortstack[l]{Inductive} & 40 W stand fan & 10/10 & 10/10\\
...
40 W stand fan & Inductive & 100\% & 100\% & 100\% & 100\%\\
...
protection paths. Sustained overload protection addressed persistent
current stress without tripping every transient. Socket-contact heating
protection responded to degraded-contact thermal behavior that current
alone could not identify.
Series arc fault protection used waveform features and context to address a
...
not included in comprehensive induced-fault coverage. The reported
false-positive and false-negative cases show that the dataset remains incomplete,
...
Certification-oriented testing was outside the present study. Creepage,
...
tests must be addressed in future studies. The implementation should be
evaluated as
...
fire-inducing electrical faults. The implementation integrates calibrated
sensing, deterministic protection logic, load-aware embedded
TinyML, relay latching, and monitoring support in one local protection
pipeline. The study also shows why the evidence must be
separated by mechanism: sustained overload requires current persistence,
degraded socket-contact heating requires conservative thermal estimation, and
series arc faults require load-aware waveform inference.

Within the tested scope, sensor errors were acceptable for the implemented
thresholds, the sustained overload function tripped the 17.30 A case after
61.3 s, degraded-contact tests showed faster and hotter socket behavior
...
normal false trip in the analysis. These findings support the feasibility
of the proposed complementary protection architecture.

Future studies should expand the normal, nuisance, and series arc fault
dataset across more residential loads, plug conditions, contact gaps, and
wiring arrangements. Longer semi-field testing should support later
