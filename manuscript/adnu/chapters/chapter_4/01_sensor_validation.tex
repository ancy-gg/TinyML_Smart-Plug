...
Chapter~\ref{chap:methodology}. In this section, the voltage, current, and NTC
temperature channels are interpreted in terms of decision reliability rather
...
test points. The corrected prototype readings produced an MAE of 3.910V, MSE
...
Table~\ref{tab:ch4-voltage-validation} lists the voltage validation points.
\begin{table}[H]
  \centering
  ...
  \begin{tabular}{ccccc}
    \toprule
    Test & Device (V) & Fluke (V) & Abs. error (V) & Squared error (V$^2$) \\
    \midrule
...
Table~\ref{tab:ch4-voltage-validation} shows that the voltage error remained
within approximately 5V across the tested range. The result supports voltage
...
The current sensor was compared with an Ingco clamp meter. The current channel
...
Table~\ref{tab:ch4-current-validation} presents the current validation points.
\begin{table}[H]
  \centering
  ...
  \begin{tabular}{ccccc}
    \toprule
    Test & Device (A) & Clamp meter (A) & Abs. error (A) & Squared error (A$^2$) \\
    \midrule
...
and sustained-trip thresholds. This supports the overload results discussed in
Section~\ref{sec:ch4-overload-protection}.

\subsection{NTC Temperature Sensor Validation}
\label{sec:ch4-temperature-validation}

The TTC05103JSY 10 k$\Omega$ thermistor was compared with a reference infrared
thermometer at the sensor measurement point. The NTC channel produced an MAE
...
direct validation of internal contact temperature. The heating tests later
show that the socket terminal can be much hotter than the external NTC reading
or compensated estimate.

Table~\ref{tab:ch4-temperature-validation} presents the thermistor validation
points.
\begin{table}[H]
  \centering
  \caption{Validation results for the NTC temperature sensor}
  \label{tab:ch4-temperature-validation}
  \begin{tabular}{ccccc}
    \toprule
    Test & Device ($^\circ$C) & IR thermometer ($^\circ$C) & Abs. error ($^\circ$C) & Squared error ($^\circ$C$^2$) \\
    \midrule
...
Table~\ref{tab:ch4-temperature-validation} supports the use of the NTC as a
stable observable input for the thermal estimator. It also reinforces the
