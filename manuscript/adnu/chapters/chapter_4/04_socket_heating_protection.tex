...
socket temperature. The important distinction is between three plotted
temperatures and one internal model state. Device temperature measurement is
the temperature measured or processed by the prototype's thermistor channel.
The infrared thermometer reading is the reference terminal or contact-region
reading. Estimated socket temperature is the compensated protection-facing
estimate derived from the characterization model. The expected normal socket
temperature is an internal comparator used by the estimator. The trip
threshold applies to the estimated socket temperature, while the infrared
thermometer reading shows how hot the socket contact area became during the
same trial.
...
limitation remains: the external thermistor channel and compensation model still
