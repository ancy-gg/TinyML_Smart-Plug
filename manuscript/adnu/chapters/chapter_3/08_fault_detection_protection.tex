...
\section{Voltage Protection}
\label{sec:ch3-voltage-protection}

Voltage protection uses calibrated RMS voltage and mains-present state. The
...
These values are summarized in Table~\ref{tab:ch3-voltage-thresholds}.
\begin{table}[H]
  \centering
  \caption{Voltage-protection thresholds verified from firmware}
  \label{tab:ch3-voltage-thresholds}
  \begin{tabular}{p{0.36\textwidth}p{0.20\textwidth}p{0.34\textwidth}}
    \toprule
    Condition & Firmware value & Interpretation \\
    \midrule
...
Table~\ref{tab:ch3-voltage-thresholds} shows that the voltage logic includes
...
\section{Socket-Heating Protection}
...
than the direct NTC temperature because the external NTC can under-read the
contact hotspot during degraded-contact heating [30], [46], [47].

\section{Series Arc-Fault Protection}
...
\section{Protection Priority and Latching}
\label{sec:ch3-priority-latching}

When multiple protection conditions are present, the firmware assigns
priorities: socket heating has priority 90, arc fault has priority 80,
sustained overload has priority 60, and voltage protection has priority 50.
The relay-latching circuit is pulsed by the firmware after a trip decision and
the fault state remains latched until the user clears it. The priority values
do not change the underlying detection equations; they determine which fault
reason is reported when conditions overlap.

The principal thresholds are summarized in Table~\ref{tab:ch3-protection-summary}.
\begin{table}[H]
  \centering
  ...
  \begin{tabular}{p{0.26\textwidth}p{0.30\textwidth}p{0.34\textwidth}}
    \toprule
    Protection function & Warning or model threshold & Relay-trip condition \\
    \midrule
    Voltage & Outside 200--250V band & Instant 170/260V or delayed 7s low/high voltage \\
...
Table~\ref{tab:ch3-protection-summary} shows that the prototype uses a hybrid
