...
collection logs are placed in Appendices~\ref{app:e} and~\ref{app:f}.

\section{Sensor-Validation Metrics}
...
\section{Overload Test Procedure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\textwidth]{ch3/figure_3.30.png}
...
\section{Socket-Heating Test Procedure}
...
external NTC and compensated estimator could provide useful warning before
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\textwidth]{ch3/figure_3.32.png}
  \caption{Testing setup block diagram for socket-heating evaluation.}
  \label{fig:ch3-heating-block}
\end{figure}

Figure~\ref{fig:ch3-heating-block} shows that both the prototype NTC and a
reference socket or terminal temperature measurement were observed during
testing. This made it possible to distinguish direct NTC temperature,
...
record time, current, direct NTC temperature, estimated socket temperature,
...
\section{Series Arc-Fault Test Procedure}
...
current features, model outputs, relay behavior, and event labels. The arc
platform is shown in Figure~\ref{fig:ch3-arc-platform}.
\begin{figure}[H]
  \centering
...
controlled arc disturbance to appear in series with the load. The platform
provided a repeatable way to create low-current gaps and restrike spikes
without depending on accidental wiring faults.

The block diagram of the series arc-fault test is shown in
Figure~\ref{fig:ch3-arc-block}.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\textwidth]{ch3/figure_3.36.png}
