...
\textit{Appendix~\ref{app:a}}; socket-heating characterization and protection data are
placed in \textit{Appendices~\ref{app:c} and~\ref{app:d}}; arc-fault testing and data
collection logs are placed in \textit{Appendices~\ref{app:e} and~\ref{app:f}}. The
letters, safety proposal, project schedule, and cost documentation are placed
in \textit{Appendices~\ref{app:g},~\ref{app:h}, and~\ref{app:i}}.
...
is shown in \textit{Figure~\ref{fig:ch3-overload-block}}.
\begin{figure}[H]
  \centering
...
\textit{Figure~\ref{fig:ch3-overload-block}} shows the prototype placed between the
...
The actual overload test setup is shown in \textit{Figure~\ref{fig:ch3-overload-actual}}.
\begin{figure}[H]
  \centering
  \includegraphics[
    width=0.75\textwidth,
...
\textit{Figure~\ref{fig:ch3-overload-actual}} documents the physical load arrangement
...
\textit{Figure~\ref{fig:ch3-heating-block}} shows the socket-heating test block
diagram.
\begin{figure}[H]
  \centering
...
\textit{Figure~\ref{fig:ch3-heating-block}} shows that both the prototype device
temperature measurement and a reference socket or terminal temperature measurement were
...
The degraded socket contacts are shown in \textit{Figure~\ref{fig:ch3-corroded-contacts}}.
\begin{figure}[H]
  \centering
  \includegraphics[
    width=0.75\textwidth,
...
\textit{Figure~\ref{fig:ch3-corroded-contacts}} documents the degraded-contact
...
\textit{Figure~\ref{fig:ch3-heating-actual}}.
\begin{figure}[H]
  \centering
  \includegraphics[
    width=0.75\textwidth,
    trim=1cm 1cm 3.2cm 10cm,
    clip
  ]{ch3/figure_3.34.png}
...
\textit{Figure~\ref{fig:ch3-heating-actual}} shows the bench configuration used to
...
in \textit{Appendix~\ref{app:c}} were used to support the thermal model, while the
protection validation data in \textit{Appendix~\ref{app:d}} were used to evaluate
relay response.
...
in \textit{Appendix~\ref{app:g}}. The arc platform is shown in
\textit{Figure~\ref{fig:ch3-arc-platform}}.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{ch3/figure_3.35.png}
  \caption{Series arc-fault test platform [48].}
  \label{fig:ch3-arc-platform}
\end{figure}

\textit{Figure~\ref{fig:ch3-arc-platform}} shows the principle of the series arc test:
...
used in this study is documented in \textit{Figure~\ref{fig:ch3-arc-actual}}.

The block diagram of the series arc-fault test is shown in
\textit{Figure~\ref{fig:ch3-arc-block}}.
\begin{figure}[H]
  \centering
...
\textit{Figure~\ref{fig:ch3-arc-block}} shows the smart plug measuring the load current
while the arc generator is placed in series with the appliance.

The actual series arc-fault test setup is shown in
\textit{Figure~\ref{fig:ch3-arc-actual}}.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{ch3/figure_3.37.png}
  \caption{Actual test setup for series arc-fault evaluation.}
  \label{fig:ch3-arc-actual}
\end{figure}

\textit{Figure~\ref{fig:ch3-arc-actual}} documents the laboratory arrangement used to
collect arc and normal trials. Complete arc-fault test records are referenced
in \textit{Appendices~\ref{app:e} and~\ref{app:f}}.
...
the negative class was normal operation. The model output was assessed according
to whether each evaluated frame was correctly or incorrectly assigned to these
two classes. The four possible outcomes are represented through a confusion
matrix, as shown in \textit{Table~\ref{tab:ch3-confusion-matrix}}.

\begin{table}[H]
  \centering
  \caption{Confusion matrix for arc-fault classification}
  \label{tab:ch3-confusion-matrix}
  \begin{tabular}{@{}p{0.25\textwidth}p{0.31\textwidth}p{0.31\textwidth}@{}}
    \toprule
    \textbf{Actual class} & \textbf{Predicted arc fault} & \textbf{Predicted normal} \\
    \midrule
    Arc fault &
    True positive (TP)&
    False negative (FN)\\
...
As shown in \textit{Table~\ref{tab:ch3-confusion-matrix}}, the confusion matrix
defines the four possible classification outcomes used in the evaluation. A true
positive (TP) represents an arc-fault frame correctly detected as an arc fault,
while a true negative (TN) represents a normal frame correctly identified as
normal. A false positive (FP) represents a normal frame incorrectly classified
as an arc fault, which corresponds to a false-trip tendency. A false negative
(FN) represents an arc-fault frame incorrectly classified as normal, which
corresponds to a missed-fault tendency.

The raw data collection produced an imbalanced distribution of normal and
arc-fault trials, as shown in \textit{Appendix~\ref{app:f}}. Because of this,
the classification evaluation in \textit{Appendix~\ref{app:e}} used balanced
normal and arc-fault trials so that accuracy would represent performance across
both classes more reliably. This step was necessary because, under an imbalanced
dataset, a model may obtain high accuracy by favoring the majority class even
when its detection of arc-fault cases is weak.
...
\end{equation}
Accuracy represents the proportion of all evaluated samples that were classified
correctly. In this study, it indicates the general correctness of the classifier
after the normal and arc-fault trials were balanced for evaluation.
...
\end{equation}
Precision measures how many arc-positive decisions were actually arc faults. A
higher precision value means that fewer normal samples were incorrectly treated
as arc faults, which is important for reducing unnecessary trip decisions during
healthy operation.
...
\end{equation}
Recall measures how many actual arc-fault samples were detected by the model. A
higher recall value means that fewer arc-fault samples were missed, which is
important because an undetected arc fault may allow the hazardous condition to
continue.
...
\begin{equation}
  ...
  \label{eq:metric-f1}
\end{equation}
The F1-score combines precision and recall into one measure. It was included
because arc-fault detection must consider both false trips and missed faults,
rather than relying on accuracy alone. These metrics are commonly used for
supervised classification and confusion-matrix evaluation [74].