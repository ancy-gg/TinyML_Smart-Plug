...
The raw calibration records are retained in \textit{Appendix~\ref{app:a},
...
\textit{Figure~\ref{fig:ch3-voltage-cal-setup}}.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\textwidth]{ch3/figure_3.17.png}
  \caption{Data-collection setup for voltage calibration.}
  \label{fig:ch3-voltage-cal-setup}
\end{figure}

\textit{Figure~\ref{fig:ch3-voltage-cal-setup}} shows the prototype voltage channel
...
\textit{Figure~\ref{fig:ch3-voltage-regression}}.
\begin{figure}[H]
  \centering
  \begin{tikzpicture}
    \begin{axis}[
...
\textit{Figure~\ref{fig:ch3-voltage-regression}} supports the cubic correction used by
...
\textit{Figures~\ref{fig:ch3-current-cal-setup} and~\ref{fig:ch3-current-cal-loads}}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{ch3/figure_3.19.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{ch3/figure_3.20.png}
  \caption{Load setups used for current calibration.}
  \label{fig:ch3-current-cal-loads}
\end{figure}

\textit{Figure~\ref{fig:ch3-current-cal-setup}} shows the comparison between the
prototype current channel and a reference instrument.
\textit{Figure~\ref{fig:ch3-current-cal-loads}} shows the different resistive and
appliance loads used to span the current range needed for overload and
arc-feature testing.

The fitted current regression is shown in
\textit{Figure~\ref{fig:ch3-current-regression}}.
\begin{figure}[H]
  \centering
  \begin{tikzpicture}
    \begin{axis}[
...
\begin{figure}[H]
  \centering
  \rotatebox{-90}{%
    \includegraphics[height=0.35\textheight]{ch3/figure_3.22.jpg}%
  }
  \caption{Data-collection setup for device temperature measurement calibration.}
  \label{fig:ch3-ntc-cal-setup}
\end{figure}

\textit{Figure~\ref{fig:ch3-ntc-cal-setup}} shows the external thermistor measurement
...
\textit{Table~\ref{tab:app-a-temperature-calibration}}.
...
The expected normal model serves as the reference condition, while the
estimated socket temperature serves as the value used for protection. This
follows the reviewed literature, which shows that poor plug-socket contact can
produce localized heating and ignition risk even when the current path still
continues to operate [17], [24], [46], [47]. The model also accounts for the
reviewed limitation that an external NTC thermistor does not directly measure
the hottest internal contact point and may respond with thermal lag [27]-[31].
For this reason, the firmware does not rely on the raw NTC reading alone.
Instead, it combines current-dependent heating behavior, ambient tracking, and
direct thermistor compensation before the protection manager evaluates the
thermal state.
...
\end{equation}
In \eqref{eq:normal-target-rise}, $0.20\degreeC$ is the normal base rise and
$12.0\degreeC$ is the normal current-rise gain. These constants define the
expected socket-temperature rise for a healthy contact at the measured load
current. This keeps the normal reference dependent on load current while still
separating expected heating from abnormal degraded-contact heating.
...
$1.20\degreeC$ is the measured current gain.
...
\textit{Table~\ref{tab:ch3-thermal-constants}}.
...
\textit{Table~\ref{tab:ch3-thermal-constants}} distinguishes the thermal estimator from
...
The calibration path for the socket-heating constants compares the device
temperature measurement and reference terminal readings against ambient, then
fits the compensation terms from the characterization records. 

This was used because the external thermistor follows the general heating of
the socket body, but it does not directly represent the hottest internal
contact point. The reference terminal reading therefore serves as the target
temperature rise, while the device temperature measurement represents the
embedded measurement available to the firmware.
...
This gain-only form is appropriate when the rise is assumed to approach zero
at ambient condition. In this case, the model only adjusts the scale of the
measured or predicted temperature rise.

When an offset is required, the corresponding linear model is
\begin{equation}
  \Delta T_{\mathrm{ref}} =
...
external thermistor rise. 
The coefficient $a$ scales the external thermistor rise, while $b$ accounts
for a fixed difference between the measured device temperature and the
reference terminal temperature. This is useful when the external thermistor
consistently under-reads the terminal region due to placement and thermal
separation.

When time lag is observed, the time series is aligned before fitting so that
the compared points represent the same stage of the heating process. This
prevents sensor delay from being treated as a permanent temperature error. The
resulting fitted constants were then used as the basis for the thermal
compensation values summarized in
\textit{Table~\ref{tab:ch3-thermal-constants}}.
...
$T_{\mathrm{comp}}=39.18\degreeC$ before the final maximum operation. 
For the same current, the normal target rise in
(\ref{eq:normal-target-rise}) is
$0.20+12.0(0.568)=7.02\degreeC$. This value does not mean that
$7.02\degreeC$ is added directly to the measured device temperature of
$38.2\degreeC$. Instead, it represents the expected steady rise above the
inferred ambient or baseline temperature for a healthy socket contact carrying
the same current. The firmware approaches this value gradually through the
normal-rise filter in (\ref{eq:normal-rise-filter}), so it is reached only
after the load has remained active long enough for the socket thermal state to
settle. For example, if the inferred ambient temperature were $30.0\degreeC$,
then the expected normal socket temperature would approach
$30.0+7.02=37.02\degreeC$ after sufficient heating time, not immediately at
the first frame. The final protection-facing estimate therefore depends on
both the direct compensated thermistor value and the filtered normal thermal
history, which prevents the method from relying only on a single instantaneous
current and device temperature reading.