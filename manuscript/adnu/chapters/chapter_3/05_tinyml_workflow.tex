...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[node distance=0.26in and 0.28in]
    \node[thesis process,text width=1.28in] (log) {Firmware CSV\\logging};
    \node[thesis process,text width=1.28in,right=of log] (label) {Manual review\\and labeling};
    \node[thesis process,text width=1.28in,right=of label] (clean) {Python cleaning\\and splits};
    \node[thesis process,text width=1.28in,below=0.30in of clean] (train) {Context and arc\\model training};
    \node[thesis process,text width=1.28in,left=of train] (report) {Metrics and\\confusion reports};
    \node[thesis process,text width=1.28in,left=of report] (export) {C headers\\exported};
    \node[thesis process,text width=1.28in,below=0.30in of export] (deploy) {Embedded\\deployment};
    \draw[thesis arrow] (log) -- (label);
    \draw[thesis arrow] (label) -- (clean);
    \draw[thesis arrow] (clean) -- (train);
    \draw[thesis arrow] (train) -- (report);
    \draw[thesis arrow] (report) -- (export);
...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[node distance=0.24in and 0.32in]
    \node[thesis process,text width=1.45in] (features) {20 firmware export\\feature fields};
    \node[thesis process,text width=1.45in,right=of features] (context) {Context model\\6 waveform inputs};
    \node[thesis process,text width=1.45in,right=of context] (arc) {Arc model\\6 waveform inputs + 7 context fields};
    \node[thesis process,text width=1.45in,below=0.32in of arc] (protect) {Persistence counter\\and relay logic};
    \node[thesis process,text width=1.45in,left=of protect] (pwa) {Live PWA\\10 feature cards};
    \draw[thesis arrow] (features) -- (context);
    \draw[thesis arrow] (context) -- (arc);
    \draw[thesis arrow] (arc) -- (protect);
    \draw[thesis arrow] (features.south) |- (pwa.west);
  \end{tikzpicture}
...
features and context fields. The live PWA dashboard displays only 10 feature
cards; a broader 14-feature order remains in the PWA metadata and session
viewer for historical review and CSV inspection.
...
context fields. Therefore, the manuscript treats the context one-hot fields
