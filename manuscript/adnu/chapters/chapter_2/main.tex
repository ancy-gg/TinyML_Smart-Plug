...
electrically global. The \textit{Development and Analysis of Electrical
...
To summarize the theoretical boundary of the study, \textit{Table 2.1} compares
...
\begin{table}[H]
  \centering
  \caption{Summary of electrical faults}
  \label{tab:ch2-fault-summary}
  \begin{tabular}{@{}p{0.18\textwidth}p{0.27\textwidth}p{0.17\textwidth}p{0.28\textwidth}@{}}
    \toprule
    \textbf{Fault class} & \textbf{Primary hazard} & \textbf{Conventional protection} & \textbf{Relevance} \\
    \midrule
    Overload & Thermal stress, insulation degradation, fire risk & Partial & Central target because branch breakers protect wiring, not necessarily the plug-socket interface. \\
    Short circuit & Immediate high-current damage and fire & Yes & Reviewed for scope delimitation; not the main target of the smart plug. \\
    Ground fault & Shock and leakage hazard & Yes & Reviewed to distinguish personnel-protection devices from the proposed socket-level fire-prevention focus. \\
    Series arc fault & Ignition-capable arcing below ordinary breaker trip levels & Not always & Central target because its current is load-limited and often waveform-dependent. \\
    High-resistance connection & Localized contact heating, glowing, melting, ignition & No & Central target because the contact can overheat without causing a classical overcurrent event. \\
    Overvoltage & Equipment damage and stress & Partial & Included as voltage supervision for supply-state interpretation. \\
    Undervoltage & Equipment stress, motor overheating, unreliable operation & Partial & Included as a supporting measurement-quality and supply-state condition. \\
    \bottomrule
  \end{tabular}
\end{table}

\textit{Table 2.1} shows that the proposed smart plug is not intended to replace
...
current-fluctuation measure. This makes {[}34{]} the proper anchor for
...
or interrupted conduction.

The same physical logic supports zero-dwell behavior around the zero-current
region. In a healthy waveform, the near-zero portion of the cycle is
relatively regular, whereas in an arc state, extinction and restrike can
increase or destabilize the time spent near zero current. {[}36{]} relies
directly on zero-crossing period analysis, and {[}34{]} likewise centers its
method on zero-current behavior.
...
40°C at an external thermistor sensor. The argument is not that all outlets
...
\textit{Table 2.2} summarizes how the reviewed literature maps to the feature and
...
\begin{table}[H]
  \centering
  \caption{Feature families relevant in the study}
  \label{tab:ch2-feature-families}
  \begin{tabular}{p{0.20\textwidth}p{0.30\textwidth}p{0.40\textwidth}}
    \toprule
    \textbf{Feature family} & \textbf{Representative quantities} & \textbf{Literature connection} \\
    \midrule
    Waveform disturbance & Midband residual ratio; mid/high-frequency spectral flux; half-cycle asymmetry; current THD; high-frequency energy delta & Arc studies report random harmonic and high-frequency disturbance, while NILM studies show that healthy loads also have family-specific spectral signatures {[}34{]}-{[}39{]}, {[}41{]}, {[}42{]}, {[}49{]}-{[}53{]}. \\
    Conduction intermittence & Low-current run time; low-current ratio; zero-dwell ratio; absolute RMS-current delta; baseline z-deviation & These quantities represent restrikes, current interruption, partial conduction, and unstable near-zero conduction timing, which are physically consistent with series arcs and weak-contact behavior {[}34{]}, {[}36{]}, {[}37{]}, {[}52{]}, {[}54{]}. \\
    Deterministic stress variables & RMS current; estimated socket temperature; voltage sag or abnormal supply condition & These variables encode thermal or electrical stress and remain essential even when machine learning is added. They improve interpretability and provide hard safety overrides for overload and heating events {[}3{]}, {[}27{]}-{[}29{]}, {[}44{]}, {[}45{]}. \\
    \bottomrule
  \end{tabular}
\end{table}

\textit{Table 2.2} shows that the proposed feature set is not arbitrary. The
