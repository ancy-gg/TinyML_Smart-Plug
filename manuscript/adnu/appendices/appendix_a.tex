\chapter{Sensor Calibration}\label{app:a}
\textbf{Sensor Calibration}

This appendix contains the calibration datasets used to derive the
correction equations for the voltage, current, and temperature sensing
channels. These tables support the sensor-calibration procedures
described in Chapter 3 and the validation results discussed in Chapter
4.

\textbf{Table A.1.} Voltage Sensor Calibration Data

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2138}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2540}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2203}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2331}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
\textbf{ZMPT101B}

\textbf{(V)}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\textbf{Fluke}

\textbf{Multimeter (V)}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\textbf{ZMPT101B}

\textbf{(V)}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\textbf{Fluke}

\textbf{Multimeter (V)}
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
34.1
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
35.2
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
179.1
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
180.5
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
43.9
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
44.79
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
189
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
190
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
58.6
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
60.1
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
199.2
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
200.4
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
73
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
74.5
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
204.7
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
205.3
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
83.3
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
85
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
210.7
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
210
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
99.3
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
100.2
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
214.2
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
214.8
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
113.6
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
115.1
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
220
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
220.2
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
129
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
130
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
225.3
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
225.1
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
139.6
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
140.3
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
228.4
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
230.8
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
149
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
150
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
235.3
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
235.1
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
159.1
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
160
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
238.8
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
240.7
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
169.2
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
169.9
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
242.9
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
244.6
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\end{longtable}
}

\textbf{Table A.2.} Current Sensor Calibration Data

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2074}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2637}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2042}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2460}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
\textbf{ACS37035}

\textbf{(A)}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\textbf{Ingco Clamp}

\textbf{Meter (A)}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\textbf{ACS37035}

\textbf{(A)}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\textbf{Ingco Clamp}

\textbf{Meter (A)}
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
0.08
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
0.08
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
3.28
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
4.09
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
0.15
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
0.15
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
3.31
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
4.15
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
0.17
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
0.18
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
3.86
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
4.7
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
0.19
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
0.2
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
3.97
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
4.9
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
0.21
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
0.22
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
5.89
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
7.27
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
0.53
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
0.64
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
4.62
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
5.72
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
0.56
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
0.66
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
4.84
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
6.11
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
0.58
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
0.69
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
4.95
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
6.22
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
0.63
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
0.77
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
5.04
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
6.33
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
0.73
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
0.86
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
5.11
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
6.46
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
0.76
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
0.88
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
5.33
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
6.6
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
0.7
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
0.89
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
5.36
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
6.76
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
0.76
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
1.02
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
5.41
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
6.88
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
1.55
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
1.9
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
6.51
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
8.12
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
1.58
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
1.91
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
6.79
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
8.35
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
1.57
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
1.94
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
6.85
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
8.48
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
1.76
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
2.18
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
7.07
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
8.8
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
1.77
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
2.18
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
8.24
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
10.32
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
1.96
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
2.32
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
8.35
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
10.46
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
2.02
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
2.47
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
8.41
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
10.47
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
2.2
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
2.77
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
9.4
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
12.2
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
3.12
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
3.82
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
10.3
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
13.3
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
3.11
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
3.87
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
12.4
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
16
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\end{longtable}
}

\textbf{Table A.3.} Tabulation of Reference Temperature and Thermistor
Resistance

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2203}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2846}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2749}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1479}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
\textbf{Reference Reading (°C)}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\textbf{Absolute Temperature}

\textbf{(K)}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\textbf{Measured Thermistor Resistance (Î©)}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\textbf{ln(R)}
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
38
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
311.15 K
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
5.8k
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
8.66072
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
50
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
323.15 K
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
3.6k
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
8.18869
\end{minipage} \\
\begin{minipage}[b]{\linewidth}\centering
100
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
373.15 K
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
0.7k
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
6.55108
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\end{longtable}
}

