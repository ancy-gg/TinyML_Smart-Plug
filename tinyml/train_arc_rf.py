...
# Preferred 10-feature set for arc detection.
# THD stays logged in CSVs but is intentionally excluded from training.
FEATURES = [
...
def normalize_feature_names(df):
    ...
    aliases = {
        ...
        "dip_rebound_ratio": "dip_rebound_ratio",
        "thd_i": "thd_i",
...
def main():
    ...
    ap.add_argument("--fn_weight", type=float, default=250.0,
                    help="False-negative cost used when selecting the RF threshold.")
    ap.add_argument("--fp_weight", type=float, default=1.0,
                    help="False-positive cost used when selecting the RF threshold.")
    args = ap.parse_args()
