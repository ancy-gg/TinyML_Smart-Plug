...
def normalize_feature_names(df):
    ...
    aliases = {
        ...
        "pre_dip_spike_ratio": "pre_dip_spike_ratio",
        "thd_i": "thd_i",
        "pulse_count_per_cycle": "pulse_count_per_cycle",
    }

    rename_map = {c: aliases[str(c).strip()] for c in df.columns if str(c).strip() in aliases}
    if rename_map:
...
def main():
    ...
    with open(args.out_header, "w", encoding="utf-8", newline="\n") as f:
        f.write("#pragma once\n")
