...
OUT_HEADER = str(PROJECT_ROOT / "tinyml" / "model" / "TinyMLTreeEnsemble_ET.h")
OUT_JOBLIB = str(PROJECT_ROOT / "tinyml" / "trainer" / "lib" / "TinyMLTreeEnsemble_ET.joblib")
...
def resolve_arc_feature_names(explicit_features=None, feature_report: str | None = None, model_key: str = "et", *, allow_legacy_default_report: bool = True) -> tuple[list[str], list[str]]:
...
def main():
    ...
    ap.add_argument("--search_profile", default="final_et")
    args = ap.parse_args()
    ...
    if args.generation:
        ...
        apply_generation_defaults(
            ...
            argv,
            {
                ...
                "out_report": ("--out_report", generation_paths.et_report),
    ...
    selected_base_features, selected_feature_names = resolve_arc_feature_names(args.features, args.feature_report, model_key="et", allow_legacy_default_report=(selected_generation is None))
    ...
    if generation_paths is not None:
        archived = archive_generation_exports(generation_paths, kinds=["et"])
        update_generation_manifest(
            generation_paths,
            {
                "selected_generation": selected_generation,
                "source_model_generations_seen": sorted(result["source_model_generation_summary"].keys()),
                "trainer_settings_snapshot": {"et": settings},
                "et_training": {
                ...
                "recommended_next_action": (
                    ...
                    f"Canonical Extra Trees export remains available for comparison. "
                    + (
